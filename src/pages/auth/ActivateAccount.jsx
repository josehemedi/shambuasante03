...
export default function ActivateAccount() {
  ...
  async function handleSubmit(e) {
    ...
    if (password.length < 8) {
      setError(t("auth.errorPasswordTooShort") || "Le mot de passe doit comporter au moins 8 caractères.")
      return
    }
    if (password !== confirm) {
    ...
    setLoading(true)
    try {
      await activateAccountRequest({ token, password })
      setDone(true)
      setTimeout(() => navigate("/login", { replace: true }), 2200)
    } catch (err) {
      setError(err?.payload?.message || err?.message || t("auth.errorActivateFailed"))
      setLoading(false)
    }
  }
    ...
    return (
      <AuthShell>
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">{t("auth.activateInvalidHint")}</p>
          <Link
            to="/login"
        ...
        {done ? (
          <div className="text-center">
            ...
            <h2 className="mt-5 font-display text-2xl font-bold tracking-tight text-foreground">
              {t("auth.activateDoneTitle")}
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground text-pretty">
              {t("auth.activateDoneBody")}
            </p>
                ...
                {password && (
                  <div className="mt-2">
                    ...
                    <p className="mt-1 text-[11px] font-medium text-muted-foreground">
                      ...
                      <span className="text-foreground">{strengthLabels[strength]}</span>
                    </p>
                  </div>
                )}
              </div>
